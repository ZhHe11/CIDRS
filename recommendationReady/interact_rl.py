...
resume = '../SaveModel/ObjEpoch_50.pt'
...
def get_acc2nd(batch_size, G_ids, G_ids2nd, T_ids, is_success2nd, turn=-1):
    acc_in_turn2nd = 0
    for i in range(batch_size):
        if is_success2nd[i] == 1:
            continue
        elif G_ids[i] == T_ids[i] or G_ids2nd[i] == T_ids[i]:
            acc_in_turn2nd += 1
            is_success2nd[i] = 1
    return acc_in_turn2nd, is_success2nd
...
def show_img(img_id):
    img_path = "../fur/processed_img/" + str(img_id)[1:-1] + ".jpg"
...
def main():
    ...
    # load objs
    with open('./datajson/dict-real-train.json', 'r') as f:
        load_dict_train_r = json.load(f)
    ...
    load_model_name = '/home/data/zh/project1/streamlit/output/' + 'F018/F018-2p-t8-1-r' + '.pth'
    # load_model_name = '/home/data/zh/project1/streamlit/output/' + 'F015/F015-1p-t8-g' + '.pth'
    # load_model_name = '/home/data/zh/project1/streamlit/output/' + 'F015/F015-1p-t6-g' + '.pth'

    # load_model_name = '/home/data/zh/project1/streamlit/output/' + 'F017/F017-2p-t6-20-g' + '.pth'
    # load_model_name = '/home/data/zh/project1/streamlit/output/' + 'F015/F015-1p-t8-r' + '.pth'
    # load_model_name = '/home/data/zh/project1/streamlit/output/' + 'F013/F013-real-train-1p' + '.pth'
    # load_model_name = '/home/data/zh/project1/streamlit/output/' + 'F016/F016-1p-r-g' + '.pth'
    ...
    for episode in range(EPISODES):
        ...
        if update_train == 0:
            ...
            print("ranker_train update_emb time: ", time.time() - time1)
            update_train = 1
        net, acc_max = train(dataloader, device1, batch_size, net, ranker, load_dict_train, turns, num_actions, model_name,
                             episode, acc_max, net_batch, isReal=False)
        # test process
        if update_test == 0:
            time1 = time.time()
...
def train(dataloader, device1, batch_size, net, ranker, load_dict, turns, num_actions, model_name, episode, acc_max, net_batch, isTest=False, isReal=False, acc_test_max=0):
    ...
    for data in dataloader:
        ...
        is_success2nd = np.zeros(len(T_ids))
        ...
        if isReal:
            g_txt, t_txt, fb_txt = get_feedback_from_json_real(T_ids, T_ids, turn=-1, load_dict=load_dict,
        ...
        G_ids2nd = C_ids[:, 1].cpu().numpy()
        G_imgs = C_imgs[:, 0]
        G_pre = C_pre[:, 0]
        G_imgs_his = torch.zeros((batch_size, lensOfFeedbacks, 512)).to(device1)
        G_imgs_his[:, 0] = G_imgs
        hx_in_turn = f_embed.detach()
        if isReal:
            g_txt, t_txt, fb_txt = get_feedback_from_json_real(G_ids, T_ids, turn=0, load_dict=load_dict,
                                                          sentence_clip_model=sentence_clip_model, G_pre=G_pre,
                                                          T_pre=T_pre)
        else:
            g_txt, t_txt, fb_txt = get_feedback_from_json(T_ids, T_ids, turn=0, load_dict=load_dict,
                                                          sentence_clip_model=sentence_clip_model, G_pre=G_pre,
                                                          T_pre=T_pre)
        ...
        acc_in_turn2nd, is_success2nd = get_acc2nd(batch_size, G_ids, G_ids2nd, T_ids, is_success2nd)
...
##################################################### 3. get action ###################################################
            ...
            if turn == turns - 1:
                ...
                _, indexes = p.topk(2, dim=-1)
                max_index = indexes[:, 0]
                index_2nd = indexes[:, 1]

                # max_index = torch.zeros(batch_size).long().to(device1)
                # index_2nd = torch.zeros(batch_size).long().to(device1)
                # for batch_i in range(batch_size):
                #     random_indexes = random.sample(range(num_actions), 2)
                #     max_index[batch_i] = 0
                #     max_index[batch_i] = random_indexes[0]   # if greed search, max_index[batch_i] = 0
                    # index_2nd[batch_i] = 1
                    # index_2nd[batch_i] = random_indexes[1]   # if greed search, max_index[batch_i] = 1
            ...
            G_2nd_ids = torch.zeros(batch_size).to(device1)
            for i in range(batch_size):
                # # random action
                # index = random.randint(0, ranker.data_id.shape[0] - 1)
                # G_next_ids[i] = ranker.data_id[index]
                # index2nd = random.randint(0, ranker.data_id.shape[0] - 1)
                # G_2nd_ids[i] = ranker.data_id[index2nd]
                ...
                G_2nd_ids[i] = C_ids[i][index_2nd[i]]        # [16]

            G_next_ids = G_next_ids.cpu().numpy().astype(int)
            G_next_ids_2nd = G_2nd_ids.cpu().numpy().astype(int)
            G_imgs_his_next = G_imgs_his
            G_imgs_his_next[:, turn + 1] = G_next_imgs
            G_next_index = torch.zeros((batch_size, 1)).to(device1)
            for i in range(batch_size):
                for j in range(len(ranker.data_id)):
                    if ranker.data_id[j] == G_next_ids[i]:
                        G_next_index[i] = j
                        break
            # 4. get next state
            if isReal:
                g_next_txt, t_next_txt, fb_next_txt = get_feedback_from_json_real(
            ...
            acc_in_turn2nd, is_success2nd = get_acc2nd(batch_size, G_next_ids, G_next_ids_2nd, T_ids, is_success2nd)
            # 6. store transition
            if isTest == False:
                f_embed_tg = torch.cat((f_embed_his_t, f_embed_his_g), dim=1)
                distance_tg = torch.cat((distance_t, -distance_g), dim=1)
                f_embed_tg_next = torch.cat((f_embed_his_t_next, f_embed_his_g_next), dim=1)
                distance_tg_next = torch.cat((distance_next_t, distance_next_g), dim=1)
                net.store_transition(G_imgs_his, f_embed_tg, distance_tg, max_index, reward,
                                     G_imgs_his_next, f_embed_tg_next, distance_tg_next,
                                     G_next_index,
                                     batch_size=batch_size, net_mem=20, success_turn=is_success, turn=turn)
                actor_loss, critic_loss = net.learn(batch_size=net_batch, device=device1, ranker=ranker, k=num_actions)
        ...
        print("r@1: %.4f" % (acc_all / (batch_count * batch_size)),
              "AverageSuccessTurn: %.4f" % (np.array(AST).mean()),
              "r@2: %.4f" % (acc_all2nd / (batch_count * batch_size) + acc_all / (batch_count * batch_size)),
              "loss_actor:", loss_in_turn_a / turns, "loss_critic:", loss_in_turn_c / turns)

    # save result to txt
    acc_all = acc_all / (batch_count * batch_size)
    acc_all2nd = acc_all2nd / (batch_count * batch_size) + acc_all
    ...
    with open("../output/" + model_name + ".txt", "a") as f:
        if isTest == True:
            f.write('[' + model_name + ']' + str(episode) + " r@1: " + str(acc_all) + " r@2: " + str(acc_all2nd) + "\n")
        else:
            f.write("train: " + str(episode) + " acc: " + str(acc_all) + "\n")
    # save model
    print("[episode: ", str(episode), "] r@1: ", str(acc_all),
          " r@2: ", str(acc_all2nd), " acc_max: ", str(acc_max))
    state = {
        'episode': episode + 1,
